...
(() => {
  ...
  async function guardar(){const p=parametrosFormulario(),e=document.getElementById('estadoAdminAlumnos'),b=document.getElementById('btnGuardarAdminAlumno');if(!p.nombre||!p.apellidoPaterno||!p.grado||!p.grupo){e.textContent='❌ Completa nombre, apellido paterno, grado y grupo.';return;}b.disabled=true;e.textContent='⏳ Guardando alumno...';try{const accion=alumnoSeleccionado?'actualizaralumno':'crearalumno';const guardarConFotografia=window.AulaNFCAdministracionAlumnos?.guardarAlumnoConFotografia;const r=typeof guardarConFotografia==='function'?await guardarConFotografia(accion,p,solicitarJSONP):await solicitarJSONP(accion,p);if(!r||(r.ok===false||r.exito===false))throw new Error(r?.mensaje||'No fue posible guardar el alumno.');e.textContent=`✅ ${r.mensaje||'Alumno guardado.'}`;document.getElementById('formAdminAlumno').classList.add('oculto');limpiarFormulario();await cargarAlumnos(true);}catch(err){e.textContent=`❌ ${err?.message||'No fue posible guardar el alumno.'}`;}finally{b.disabled=false;}}
